...
- Git 默认层仍包含仅供开发演示使用的明文账号密码，且仓库为公开仓库。本次迁移继续以忽略目录中的完整生产 `users.json` 为服务器真值，不调整现有认证格式。
- 后续需单独设计凭据治理方案，包括确认演示凭据绝不复用于生产、轮换任何曾复用的生产凭据，并评估密码哈希或外部秘密注入与用户管理页面的兼容改造。
- `verify-production-baseline.cjs` 继续不比较、不输出密码字段，避免把生产秘密带入日志或审查附件；这不替代后续安全专题。
